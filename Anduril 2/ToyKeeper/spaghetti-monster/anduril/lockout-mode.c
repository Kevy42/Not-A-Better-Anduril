...
uint8_t lockout_state(Event event, uint16_t arg) {
    ...
    if (event == EV_enter_state) {
        ...
        button_led_current_state = button_led_lockout_mode; // MODIFICATION
    ...
    // 6 clicks: change button LED mode // MODIFICATION
        ...
        uint8_t mode = button_led_lockout_mode; // MODIFICATION
        mode = (mode + 1) % 3; // MODIFICATION
        button_led_lockout_mode = mode; // MODIFICATION
