...
uint8_t off_state(Event event, uint16_t arg) {
    ...
    if (event == EV_enter_state) {
        ...
        #endif
        #if defined(USE_BUTTON_LED) // MODIFICATION
        button_led_current_state = button_led_off_mode; // MODIFICATION
        button_led_set(button_led_current_state); // MODIFICATION
        #endif // MODIFICATION
        #ifdef USE_SUNSET_TIMER
    ...
    // 4 clicks: soft lockout
        ...
        // Only flash main leds for lights that dont have button/aux leds. // MODIFICATION
        // Not including indicator leds as i'm not sure of they're behaviour, nor do i care. // MODIFICATION
        #if !defined(USE_BUTTON_LED) && !defined(USE_AUX_RGB_LEDS) // MODIFICATION
            blink_once(); // MODIFICATION
        #endif // MODIFICATION
    ...
    // MODIFICATION (Entire code block responsible for factory reset)
    // Moved code block to be below simple UI code flow. The code itself is unchanged.
    ...
    // click, click, click, click, long-click: momentary mode
    else if (event == EV_click5_hold) { // MODIFICATION
        //blink_once();
    ...
    #endif
    #if defined(USE_BUTTON_LED) // MODIFICATION
    // 6 clicks: change button LED mode // MODIFICATION
    else if (event == EV_6clicks) { // MODIFICATION
        uint8_t mode = button_led_off_mode; // MODIFICATION
        mode = (mode + 1) % 3; // MODIFICATION
        button_led_off_mode = mode; // MODIFICATION
        button_led_current_state = mode; // MODIFICATION
        button_led_set(mode); // MODIFICATION
        save_config(); // MODIFICATION
